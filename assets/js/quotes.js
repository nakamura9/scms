...
function populatedHandler(widget){
            ...
            var quoNo = urlElements[urlElements.length- 1]; 
            $.ajax({
                url: "/invoicing/api/quote/" + quoNo + "/",
                data: {},
                method: "GET"
            }).then(res => {
                widget.setState({
...
if(populated){
    ReactDOM.render(<InvoiceTable populated={true} 
        populatedRemoveHandler={populatedRemoveHandler} 
        addHandler={addHandler} 
        removeHandler={removeHandler}
        url="/invoicing/api/quote/" />, 
    ...
    ReactDOM.render(<SearchWidget populated={true} 
        ...
        handler={searchHandler} />, document.getElementById('customer-search'));
}else{
    ReactDOM.render(<InvoiceTable populated={false} 
        addHandler={addHandler} 
        removeHandler={removeHandler}
        url="/invoicing/api/quote/" />, document.getElementById('item-table'));
